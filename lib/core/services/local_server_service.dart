...
import 'local_server_pages.dart';
...
class LocalServerService {
  ...
  Future<void> _serveImportPage(HttpRequest request) async {
    ...
    request.response.write(_cachedImportHtml ?? getImportPageHtml());
    await request.response.close();
  }
  ...
  Future<void> _serveSearchPage(HttpRequest request) async {
    ...
    request.response.headers.contentType = ContentType.html;
    final html = _cachedSearchHtml ?? getSearchPageHtml();
  ...
  Future<void> _serveLogsPage(HttpRequest request) async {
    ...
    final html = getLogsPageHtml(content);
    request.response.write(html);
    await request.response.close();
  }
  ...
  Future<void> _serveWebDAVConfigPage(HttpRequest request) async {
    request.response.headers.contentType = ContentType.html;
    request.response.write(_cachedWebDAVConfigHtml ?? getWebDAVConfigPageHtml());
    await request.response.close();
  }
  ...
  Future<void> _serveUserAgentPage(HttpRequest request) async {
    request.response.headers.contentType = ContentType.html;
    request.response.write(_cachedUserAgentHtml ?? getUserAgentPageHtml());
    await request.response.close();
  }
