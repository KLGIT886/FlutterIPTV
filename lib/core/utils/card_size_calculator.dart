...
class CardSizeCalculator {
  ...
  static int calculateCardsPerRow(double availableWidth) {
    ...
    if (PlatformDetector.isMobile) {
      ...
      // ServiceLocator.log.d('频道页卡片计算 - 手机端: 宽度=${availableWidth.toStringAsFixed(1)}px, 每行=$cardsPerRow张', tag: 'CardSize');
      return cardsPerRow;
    } else if (PlatformDetector.isTV) {
  ...
  static int calculateHomeCardsPerRow(double availableWidth) {
    ...
    if (PlatformDetector.isMobile) {
      ...
      // ServiceLocator.log.d('首页卡片计算 - 手机端: 宽度=${availableWidth.toStringAsFixed(1)}px, 每行=$cardsPerRow张', tag: 'CardSize');
      return cardsPerRow;
    } else if (PlatformDetector.isTV) {
