...
/// Dialog for configuring WebDAV via QR code
class QrWebdavConfigDialog extends StatefulWidget {
  final Function(String serverUrl, String username, String password) onConfigReceived;
  
  const QrWebdavConfigDialog({
    super.key,
    required this.onConfigReceived,
  });

  @override
  State<QrWebdavConfigDialog> createState() => _QrWebdavConfigDialogState();
}


class _QrWebdavConfigDialogState extends State<QrWebdavConfigDialog> {
  ...
  @override
  void dispose() {
    _serverService.onWebDAVConfigReceived = null;
    _serverService.stop();
    super.dispose();
  }
  ...
  Future<void> _startServer() async {
    ...
    // 设置配置接收回调
    _serverService.onWebDAVConfigReceived = (serverUrl, username, password) {
      ServiceLocator.log.i('收到 WebDAV 配置', tag: 'QrWebdavConfigDialog');
      ServiceLocator.log.d('服务器地址: $serverUrl', tag: 'QrWebdavConfigDialog');
      ServiceLocator.log.d('用户名: $username', tag: 'QrWebdavConfigDialog');
      ...
      if (mounted) {
        ...
        // 调用回调
        widget.onConfigReceived(serverUrl, username, password);
  ...
  Widget _buildQrCodeState(AppStrings strings, bool isMobile) {
    // 生成 WebDAV 配置 URL
    final configUrl = '${_serverService.serverUrl}/webdav-config';
          ...
          Container(
            ...
            child: Column(
              children: [
                ...
                _buildStep('2', strings.webdavConfigStep2),
                const SizedBox(height: 8),
                _buildStep('3', strings.webdavConfigStep3),
              ],
            ),
          ),
